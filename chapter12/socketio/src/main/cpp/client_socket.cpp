...
std::map<std::string,napi_threadsafe_function*> maps;
    ...
    bool needAck, sio::message::list &ack_message)
{
    ...
    if (on_event_listener_call_aux_ref != nullptr) {
        ...
        if (name == "new message") {
            ...
            data->result = message->get_string();
            napi_threadsafe_function* g_function = maps[name];
            napi_acquire_threadsafe_function(*g_function);
            // 调用主线程函数，传入 Data
            napi_call_threadsafe_function(*g_function, data, napi_tsfn_blocking);
//             napi_acquire_threadsafe_function(g_tsfnOnCommonCall);
//             // 调用主线程函数，传入 Data
//             napi_call_threadsafe_function(g_tsfnOnCommonCall, data, napi_tsfn_blocking);
        }
...
public:
    ...
    void on_emit_callback(std::string const &ack_name, sio::message::list const &list)
    {
        ...
        if (on_emit_listener_call_ref_map.empty() || ack_name.empty() || list.size()==0 || list.at(0)==nullptr) {
            return;
        }
...
static napi_value on(napi_env env, napi_callback_info info)
{
    ...
    if (strcmp(eventName, g_newMessage) == 0) {
        ...
        napi_threadsafe_function* g_function = new napi_threadsafe_function();
        maps[std::string(eventName)] = g_function;
        NapiCreateThreadsafe(env,  on_event_listener_call_aux, CallJsEmit,
            g_function);
//         NapiCreateThreadsafe(env,  on_event_listener_call_aux, CallJsEmit,
//             &g_tsfnOnCommonCall);
...
static napi_value emit(napi_env env, napi_callback_info info)
{
    ...
    if (messageType == napi_string) {
        char message[MAX_BUF_SIZE+65000];
        napi_get_value_string_utf8(env, args[1], message, MAX_BUF_SIZE+65000, &result);
        bool isArraybuffer;
